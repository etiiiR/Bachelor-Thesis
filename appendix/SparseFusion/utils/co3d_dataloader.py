...
CO3D_ALL_TEN = ['donut', 'apple', 'hydrant', 'vase', 'cake', 'ball', 'bench', 'suitcase', 'teddybear', 'plant']
...
class CO3Dv2Wrapper(torch.utils.data.Dataset):
    def __init__(self, root='/drive/datasets/co3d/', category='hydrant', subset='fewview_train', stage='train', sample_batch_size=20, image_size=256, masked=True, deprecated_val_region=False, return_frame_data_list = False):
        ...
        self.subset = subset
        self.stage = stage
        self.subset_lists_file: List[str] = [f"{self.dataset_root}/{category}/set_lists/set_lists_{subset}.json"]
        ...
        if 'all_' in category or category == 'all':
            ...
            if category == 'all':
                ...
                cats = ['donut', 'apple', 'hydrant', 'vase', 'cake', 'ball', 'bench', 'suitcase', 'teddybear', 'plant']
            elif category == 'all_15':
                cats = ['hydrant', 'teddybear', 'motorcycle', 'bench', 'hotdog', 'remote', 'suitcase', 'donut', 'plant', 'toaster', 'keyboard', 'handbag', 'toyplane', 'tv', 'orange']
            else:
    ...
    def __getitem__(self, index):
        ...
        if self.subset == 'fewview_test' and self.stage == 'test':
            ...
            batch_idx = torch.linspace(0,len(self.seq_to_frames[seq_index])-1,self.sample_batch_size).long().tolist()
        else:
        ...
        for idx in batch_idx:
            ...
            timestamp_list.append(self.frame_annots[self.seq_to_frames[seq_index][idx]]["frame_annotation"].frame_timestamp)
            frame_data_list.append(self._get_frame(int(self.seq_to_frames[seq_index][idx])))
    ...
    def _get_frame(self, index):
        ...
        if self.deprecated_val_region:
            ...
            valid_bbox[2] = torch.clip((valid_bbox[2] - torch.div(frame_data.image_size_hw[1],2,rounding_mode='floor'))/torch.div(frame_data.image_size_hw[1],2,rounding_mode='floor'), -1.0, 1.0)
            valid_bbox[3] = torch.clip((valid_bbox[3] - torch.div(frame_data.image_size_hw[0],2,rounding_mode='floor'))/torch.div(frame_data.image_size_hw[0],2,rounding_mode='floor'), -1.0, 1.0)
            # print(valid_bbox)
            ...
            if self.stage == 'train':
                ...
                max_y = valid[:,0].max()
                max_x = valid[:,1].max()
                valid_bbox = torch.tensor([min_y, min_x, max_y, max_x], device=frame_data.image_rgb.device).unsqueeze(0)
                valid_bbox = torch.clip((valid_bbox - (256//2))/(256//2), -1.0, 1.0)
                ...
                max_x = valid[:,1].max()
                valid_bbox = torch.tensor([min_y, min_x, max_y, max_x], device=frame_data.image_rgb.device).unsqueeze(0)
                valid_bbox = torch.clip((valid_bbox - (self.image_height//2))/(self.image_height//2), -1.0, 1.0)
                frame_data.valid_region = valid_bbox[0]
