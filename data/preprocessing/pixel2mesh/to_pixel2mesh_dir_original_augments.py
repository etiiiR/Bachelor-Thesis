...
if __name__ == "__main__":
    ...
    for split_name in ["train", "val", "test"]:
        ...
        for pollen_id, npz_name, dir_name, mesh_path in split_items:
            ...
            npz_path = dat_path.replace(".dat", ".npz")
            # Skip if both folder and npz already exist
            if os.path.isdir(output_model_dir) and os.path.isfile(npz_path):
                print(f"⏩ Skipping {npz_name} (already exists)")
                continue
            try:
