...
class PollenDataset(Dataset):
    def __init__(self, root_dir, input_view_num=6, target_view_num=4, total_view_n=50, fov=50, camera_rotation=True):
        ...
        self.normal_dir = self.root_dir / "normal"
        self.pose_dir = self.root_dir / "pose"
        self.fov = fov
        ...
        self.indices = sorted([int(p.stem) for p in self.rgb_dir.glob("*.png")])
        print(f"[INFO] Loaded {len(self.indices)} total views from {root_dir}")

        self.cam_poses = np.load(self.root_dir / "cameras.npz")["cam_poses"]

    def __len__(self):
        return 10000  # effectively infinite; sampling views randomly

    def load_image(self, path, bg_color):
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "input_c2ws": c2ws[:self.input_view_num],
