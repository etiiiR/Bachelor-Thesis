...
def plot_mesh(vertices, faces, title="3D Mesh", show_vertices=True, show_wireframe=True, alpha=0.7):
    """
    Plot a 3D mesh with vertices and faces.
    ...
    # Create mesh
    mesh = Poly3DCollection(face_vertices, alpha=alpha)
    mesh.set_facecolor('lightblue')
    mesh.set_edgecolor('black' if show_wireframe else 'none')
    ...
    if show_vertices:
        ax.scatter(vertices[:, 0], vertices[:, 1], vertices[:, 2], 
                  c='red', s=10, alpha=0.8, label=f'Vertices ({len(vertices)})')
